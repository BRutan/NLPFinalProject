...
# NLPFinalProject.py
#################################################
# * Object performs key steps in nlp final project.
    ...
    * Key objects/methods required for performing seminar project.
    ...
    def __init__(self, tickerInputData, database, schema = None):
        ...
        * Initialize new object.
        ...
        attrColumns = [col.lower() for col in NLPFinalProject.__CorpTableColumns.keys()]
    ...
    def CreateTables(self):
        ...
        * Create all tables to store relevant data for project. If ticker is specified, and does not exist
        ...
        # Testing:
        for table in NLPFinalProject.__tableToColumns:
            if not db.TableExists(table):
                db.CreateTable(table, NLPFinalProject.__tableToColumns[table])
        # Skip creating tables if already created:
        #if not db.TableExists("Corporations"):
        #    db.CreateTable("Corporations", NLPFinalProject.__CorpTableColumns, schema = self.__schema)
        #if not db.TableExists("Subsidiaries"):
        #    db.CreateTable("Subsidiaries", NLPFinalProject.__SubsidariesTableColumns, schema = self.__schema)
        #if not db.TableExists("CorporateBrands"):
        #    db.CreateTable("CorporateBrands", NLPFinalProject.__CorpBrandTableColumns, schema = self.__schema)
        
        # Create all Corporations tables:

        #tweetColumns = NLPFinalProject.__TweetDataColumns
        #returnColumns = NLPFinalProject.__HistoricalPriceCols
        #tweetTableSig = NLPFinalProject.__TweetTableSig
        #priceTableSig = NLPFinalProject.__PriceTableSig
        ...
        #    db.CreateTable("RetweetData", NLPFinalProject.__AllRetweetDataColumns)
    ...
    def InsertCorpAttributes(self):
        ...
        * Pull all corporate attributes for stored tickers or passed
        ...
        self.TickerToCorpAttribute = results if not results is None else DataFrame(columns = [col.lower() for col in NLPFinalProject.__CorpTableColumns])
        if tickers:
            corpID = maxCorpID + 1
            targetAttrs = [attr for attr in list(NLPFinalProject.__CorpTableColumns.keys()) if attr.lower() not in ['corpid', 'ticker']]
            puller = CorpDataPuller(targetAttrs)
            for ticker in tickers:
                columnData = { key.lower() : [] for key in NLPFinalProject.__CorpTableColumns }
    ...
    def GetSubsidiaries(self):
        ...
        * Pull subsidiaries from each corporation's 10K, and load into 
        ...
        if not results is None and not results.empty:
            ...
            cols = [col.lower() for col in NLPFinalProject.__SubsidariesTableColumns]
            cols.append('ticker')
        ...
        for ticker in toPull:
            ...
            insertData = {col.lower() : [] for col in NLPFinalProject.__SubsidariesTableColumns.keys()}
            results = []
            nameColumn = None
    ...
    def GetBrands(self):
        ...
        * Pull all brands from WIPO website, push into database.
        ...
        if not results is None and not results.empty:
            ...
            cols = [col.lower() for col in NLPFinalProject.__CorpBrandTableColumns]
            cols.append('ticker')
        ...
        for ticker in toPull:
            query = BrandQuery()
            corpId = self.TickerToCorpAttribute.loc[self.TickerToCorpAttribute.index == ticker]['corpid'].values[0]
            insertValues = { col.lower() : [] for col in NLPFinalProject.__CorpBrandTableColumns }
    ...
    def GetHistoricalData(self):
        ...
        * Get historical data for all tickers for date range specified in file.
        ...
        if not self.DB.TableExists(priceTable):
            self.DB.CreateTable(priceTable, NLPFinalProject.__HistoricalPriceCols)
        skipCols = ['corpid', 'date']
        priceTypes = [key.lower() for key in NLPFinalProject.__HistoricalPriceCols if key.lower() not in skipCols]
        ...
        for ticker in tickerToPeriod:
            ...
            if not isinstance(prices, DataFrame):
                continue
            colData = { key.lower() : [] for key in NLPFinalProject.__HistoricalPriceCols }
    ...
    def __ReadPickle(dfName):
        ...
        * Read serialized object from local pickle folder folder.
        """
        path = ''.join([NLPFinalProject.__PickleFolder, dfName, '.pickle'])
    ...
    def __InsertIntoCache(self, ticker, brand):
        ...
        * Store information regarding pulled brands for corps in local cache
        ...
        """
        val = NLPFinalProject.__cacheKeySig % (ticker, brand)
        #cache.set(val, 30000)
